...
final class NetworkService : NetworkServiceProtocol   {
  ...
  func getNews(completion: @escaping (GetNewsApiResponce) -> Void) {
    ...
    guard let url = components?.url  else {do { completion(.failure(.unknown)); return }}
    var request = URLRequest(url: url)
    request.httpMethod = "GET"
    session.dataTask(with: request) { (data, myResponce, error) in
      do {
        let data = try self.httpResponse(data: data, response: myResponce)
